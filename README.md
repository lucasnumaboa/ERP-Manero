...
- **App do Assistente (só o chat, como um WhatsApp)**: botão <i>celular</i> no cabeçalho do chat abre `frontend/assistente/`, um app instalável à parte (ícone azul "Assistente", tela cheia, login próprio com o mesmo e-mail/senha do ERP). No Android aparece "Instalar"; no iPhone, Compartilhar › Adicionar à Tela de Início. Usa o mesmo `produto-chat-widget.js` em modo tela cheia (`<body data-assistente="app">`), então a conversa é a mesma do ERP.
