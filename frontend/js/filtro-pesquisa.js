 ...
 * NOTA: Algumas páginas (como produtos.html) usam API para buscar, então
 * não devem usar a filtragem local em tempo real.
 */
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (campoPesquisa) {
        // Verifica se a página atual é uma que usa API para buscar
        // Nessas páginas, NÃO devemos adicionar o evento de input automático
        const paginasComAPI = ['produtos.html'];
        const paginaAtual = window.location.pathname.split('/').pop();

        // Só adiciona o evento de input se NÃO for uma página que usa API
        if (!paginasComAPI.includes(paginaAtual)) {
