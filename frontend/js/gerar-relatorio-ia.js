...
// Função para fazer chamada ao OpenRouter com delay
async function chamarOpenRouter(prompt, delay = 5000) {
    try {
        if (!configuracoesIA) {
...
function limparCodigoHTML(texto) {
    ...
    // Variações: pode ter espaços diferentes, quebras de linha, etc.
    texto = texto.replace(/\d+%;\s*border-collapse:\s*collapse;\s*margin:\s*[\d\w\s]+;\s*background:\s*[\w]+;\s*border:\s*[\d\w\s#,]+;["\s]*>\s*/gi, '');
    
    // Remover qualquer linha que contenha "border-collapse: collapse"
    const linhas = texto.split('\n');
    const linhasLimpas = linhas.map(linha => {
        if (linha.includes('border-collapse') || linha.includes('background: white') || linha.includes('margin: 15px')) {
            return '';
        }
        return linha;
    ...
    // Remover tags HTML abertas sem fechamento
    texto = texto.replace(/<(?:div|table|thead|tbody|tr|td|th|span|p)[^>]*>/gi, '');
    texto = texto.replace(/<\/(?:div|table|thead|tbody|tr|td|th|span|p)>/gi, '');
    
    // Remover fragmentos de HTML
    texto = texto.replace(/\d+%\s*;?\s*border-collapse[^>]*>/gi, '');
    texto = texto.replace(/[0-9a-f]{3,6}["\s]*;?\s*>\s*$/gmi, '');
    
    // Remover propriedades CSS soltas
    texto = texto.replace(/\b(?:border-collapse|background|margin|padding|width|height|color|font-weight|text-align):\s*[^;]*;/gi, '');
